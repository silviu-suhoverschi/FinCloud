...
def upgrade() -> None:
    ...
    op.add_column('users', sa.Column('theme', sa.String(length=20), server_default='auto', nullable=True))
    op.execute("UPDATE users SET theme = 'auto' WHERE theme IS NULL")
    op.alter_column('users', 'theme', nullable=False)
    op.create_check_constraint('chk_theme_valid', 'users', "theme IN ('light', 'dark', 'auto')")
