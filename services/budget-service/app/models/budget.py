...
class Budget(Base):
    ...
    # Budget Settings
    rollover_unused: Mapped[bool] = mapped_column(
    ...
    alert_threshold: Mapped[Decimal] = mapped_column(
        Numeric(5, 2), default=Decimal("80.00"), server_default="80.00"
    )
    ...
    account: Mapped["Account | None"] = relationship(
        "Account", back_populates="budgets"
    )
    spending_cache: Mapped[List["BudgetSpendingCache"]] = relationship(
        "BudgetSpendingCache", back_populates="budget", cascade="all, delete-orphan"
    )
