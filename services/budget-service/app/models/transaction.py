...
class Transaction(Base):
    ...
    # Reconciliation
    is_reconciled: Mapped[bool] = mapped_column(Boolean, default=False, server_default="false")
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            "exchange_rate > 0",
            name="chk_exchange_rate_positive"
        ),
        Index("idx_transactions_user_id", "user_id", postgresql_where="deleted_at IS NULL"),
        Index("idx_transactions_account_id", "account_id", postgresql_where="deleted_at IS NULL"),
        Index("idx_transactions_category_id", "category_id"),
        Index("idx_transactions_date", "date", postgresql_ops={"date": "DESC"}),
        Index("idx_transactions_type", "type"),
        Index("idx_transactions_created_at", "created_at", postgresql_ops={"created_at": "DESC"}),
